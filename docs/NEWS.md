...
- **RSS 失败**：日志里会列出失败源，详情见 `news/.state/feed-health.json`。偶发 429/500 属源站限流或故障；`feed-health-summary` 在 prebuild 末尾以 `--soft` 打印告警（不阻断部署）。本地硬校验可用 `npm run feed-health -- --warn-threshold 5`。
